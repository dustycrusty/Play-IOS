//
//  regulationCheckShortPage.swift
//  playBetaV1
//
...
import M13Checkbox
import Firebase
class regulationCheckShortPage: UIViewController {
    ...
    func navigationToFinalScreen(){
        if Auth.auth().currentUser == nil {
        Auth.auth().createUser(withEmail: email!, password: idToken!) { (user, error) in
            if (error != nil)
            {
                if let errorCode = AuthErrorCode(rawValue: (error?._code)!){
                    ...
                    case .emailAlreadyInUse:
                        print("creating user..")
                        let storyboard = UIStoryboard(name: "Main", bundle: nil)
                        let vc = storyboard.instantiateViewController(withIdentifier: "submitPage_Short")
                        if Auth.auth().currentUser?.displayName != nil{
                            createUserDataRef(uid: (Auth.auth().currentUser?.uid)!, username: (Auth.auth().currentUser?.displayName)!)
                        }
                        else{
                            createUserDataRef(uid: (Auth.auth().currentUser?.uid)!, username: " ")
                        }
                        self.navigationController?.pushViewController(vc, animated: true)
                    default:
                         errorPopup(vc: self, error: error!, errorName: "유저 생성 에러")
                    }
                }
